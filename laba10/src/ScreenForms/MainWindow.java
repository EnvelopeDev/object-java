 ...
 * @version 2.0 (with integrated multithreading and logging)
 */
public class MainWindow
{
    ...
    public void show()
    {
        log.info("Showing main application window");
    ...
    public MainWindow() throws InputException
    {
        ...
        log.info("Initializing Dog Festival application");
        ...
        log.info("Main window initialization completed successfully");
    }
    ...
    {
        log.debug("Starting data initialization from XML: " + DOGS_FILE_PATH);
        ...
        fileMngr.loadFromXML(DOGS_FILE_PATH, new FileManager.FileOperationCallback<List<Dog>>() {
            ...
            public void onSuccess(List<Dog> result) {
                dogs = result;
                log.info("Successfully loaded " + dogs.getSize() + " dogs from XML");
                latch.countDown();
            }
            
            @Override
            public void onError(Exception e) {
                log.error("Error loading data from XML: " + e.getMessage(), e);
                loadException[0] = e;
                latch.countDown();
            }
        });
        ...
        try {
            ...
            log.warn("Data loading thread was interrupted", e);
    ...
    {
        log.debug("Initializing application fonts");
    ...
    {
        log.debug("Initializing buttons panel");
    ...
    {
        log.debug("Initializing data table");
        ...
        log.info("Table initialized with " + dogs.getSize() + " rows");
    }
    ...
    {
        log.debug("Initializing search panel");
    ...
    {
        log.debug("Assembling main frame components");
    ...
    {
        log.debug("Restoring all data to table");
        tableModel.setRowCount(0);
        for (String[] row : originalTableData)
        {
            tableModel.addRow(row);
        }
        log.info("Restored " + originalTableData.length + " rows");
    }
    ...
    {
        log.debug("Updating table with " + data.getSize() + " rows");
        tableModel.setRowCount(0);
    ...
    private static void handleButtonClick(int buttonIndex)
    {
        ...
        log.info("Button clicked: " + buttonName + " (index: " + buttonIndex + ")");
    ...
    private static void saveData() {
        log.info("Starting multithreaded save operation");
        final CountDownLatch saveCompleteLatch = new CountDownLatch(1);
        
        Thread thread1 = new Thread(() -> {
            log.debug("Thread 1 started: Saving to XML");
            fileMngr.saveToXML(DOGS_FILE_PATH, dogs, new FileManager.FileOperationCallback<Void>() {
                @Override
                public void onSuccess(Void result) {
                    log.info("Thread 1: Data saved successfully to " + DOGS_FILE_PATH);
                    saveCompleteLatch.countDown();
                }
                @Override
                public void onError(Exception e) {
                    log.error("Thread 1: Error saving data: " + e.getMessage(), e);
                    saveCompleteLatch.countDown();
                }
            });
        });
        ...
        Thread thread2 = new Thread(() -> {
            try {
                log.debug("Thread 2: Waiting for save completion...");
                saveCompleteLatch.await();
                log.debug("Thread 2: Starting HTML report generation");
                ReportGenerator.generateHTMLReport(DOGS_FILE_PATH,"dog_report.html");
            } catch (InterruptedException e) {
                log.warn("Thread 2 was interrupted", e);
                Thread.currentThread().interrupt();
            }
        });
        ...
        Thread thread3 = new Thread(() -> {
            try {
                log.debug("Thread 3: Waiting for save completion...");
                saveCompleteLatch.await();
                log.debug("Thread 3: Starting PDF report generation");
                ReportGenerator.generatePDFReport(DOGS_FILE_PATH,"dog_report.pdf");
            } catch (InterruptedException e) {
                log.warn("Thread 3 was interrupted", e);
                Thread.currentThread().interrupt();
            }
        });
    ...
    private static Thread threadsControl(Thread thread1, Thread thread2, Thread thread3)
    {
    	return new Thread(() -> {
            try {
                log.debug("Control thread: Waiting for all threads to complete");
                thread1.join();
                thread2.join();
                thread3.join();
                
                log.info("All save threads completed successfully");
                ...
                log.error("Control thread was interrupted", e);
                Thread.currentThread().interrupt();
            }
        });
    }
    ...
    {
        log.debug("Displaying exit confirmation dialog");
        ...
        {
            log.info("User confirmed exit - saving data and shutting down");
            try {
                fileMngr.outputToXML(DOGS_FILE_PATH, dogs);
                log.info("Data saved successfully before exit");
            } catch (IOException e) {
                log.error("Error saving data before exit: " + e.getMessage(), e);
                e.printStackTrace();
            }
    ...
    private static void searchElement(String searchText)
    {
        ...
        {
            log.debug("Empty search text - restoring all data");
            restoreData();
            return;
        }
        
        searchText = searchText.toLowerCase().trim();
        log.debug("Searching for: '" + searchText + "' in " + originalTableData.length + " rows");
        ...
        log.info("Search completed: found " + findedData.getSize() + " matching rows");
